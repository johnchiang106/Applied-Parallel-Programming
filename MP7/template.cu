...
__global__ void spmvJDSKernel(float *out, int *matColStart, int *matCols,
  ...
  if (x_in < dim) {
    float dot = 0;
    int row_end = matRows[x_in];
    for (int elem = 0; elem < row_end; ++elem) {
      int pos = matColStart[elem] + x_in;
      dot += matData[pos] * vec[matCols[pos]];
    }
    out[matRowPerm[x_in]] = dot;
  }
}
...
int main(int argc, char **argv) {
  ...
  free(hostCSRCols); free(hostCSRRows); free(hostCSRData);
  free(hostVector); free(hostOutput); free(hostJDSColStart);
  free(hostJDSCols); free(hostJDSRowPerm); free(hostJDSRows); free(hostJDSData);
