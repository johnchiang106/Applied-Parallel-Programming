...
__global__ void FloatToUChar(float* imgIn, uint8_t* imgOut, int imgSize){
  int idx = bd * bx + tx;
  if(idx < imgSize)
    imgOut[idx] = (uint8_t) ((HISTOGRAM_LENGTH - 1) * imgIn[idx]);
}

__global__ void UCharToGray(uint8_t* imgIn, uint8_t* imgOut, int imgSize){
  int idx = bd * bx + tx;
  if(idx < imgSize){
    uint8_t red, grn, blu;
    ...
    imgOut[idx] = (uint8_t) ((0.21 * red) + (0.07 * blu) + (0.71 * grn));
  }
}
...
int main(int argc, char **argv) {
  ...
  //@@ launch each kernel
  FloatToUChar<<<grid, block>>>(deviceIn, deviceUChar, imageWidth * imageHeight * imageChannels);
  UCharToGray<<<grid, block>>>(deviceUChar, deviceGray, imageWidth * imageHeight);
